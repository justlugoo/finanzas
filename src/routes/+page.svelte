<script lang="ts">
  ...
  import "../app.css";
  import type { PeriodSummary, CategoryProgress, Transaction } from "$lib/types";
  ...
  $effect(() => {
    const period = activePeriod; // dependencia reactiva
    let cancelled = false;
    ...
    async function load() {
      ...
      // Esperar DB (reintenta si no está lista)
      while (!cancelled) {
        try {
...
<main>
  ...
  <section class="kpis">
    ...
    <div class="kpi-card" class:balance-pos={!loading && (summary?.balance ?? 0) >= 0} class:balance-neg={!loading && (summary?.balance ?? 0) < 0}>
        ...
        {#each categories as cat}
          ...
          <li class="category-row">
            ...
            <div class="bar-track">
              <div
                class="bar-fill"
                class:bar-over={cat.is_over}
                style="width: {pct}%"
              ></div>
            </div>
            {#if cat.monthly_target > 0}
              <span class="cat-pct" class:over={cat.is_over}>
